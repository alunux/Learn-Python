...
def main(argument):
    ...
    if len(nim) == 15:
        if int(nim) % 2 == 0:
            print "NIM Anda Genap: ", nim
            print "Sorted NIM (MIN->MAX):", \
                  "".join(map(str, sort_min_to_max(map(int, nim))))
            print "Sorted NIM (MAX->MIN):", \
                  "".join(map(str, sort_max_to_min(map(int, nim))))
        else:
            print "NIM Anda Ganjil: ", nim
